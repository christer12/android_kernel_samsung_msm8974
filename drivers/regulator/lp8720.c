	...
	u8 *dest)
{
	...
	if (ret < 0) {
		dev_err(&i2c->dev, "lp8720_i2c_read failed (%d)\n", ret);
		return ret;
	}
	...
	const u8 value)
{
	...
	if (ret < 0) {
		dev_err(&i2c->dev, "lp8720_i2c_write failed (%d)\n", ret);
	}
	...
	struct lp8720_platform_data *pdata)
{
	...
	for (i = 0; i < pdata->num_regulators; i++) {
		...
		lp8720->rdev[i] = regulator_register(&regulators[reg->id],
					lp8720->dev, reg->initdata, lp8720, NULL);
			    ...
			    const struct i2c_device_id *id)
{
	...
	lp8720_i2c_read(i2c, LP8720_ENABLE_REG, 1, &readbyte);
#if defined(CONFIG_MACH_MONTBLANC) || (CONFIG_MACH_VIKALCU)
