					  ...
					  int adc, int chgtyp)
{
	...
	case ADC_HMT:
		...
		if (mdata->dock_cb)
			mdata->dock_cb(MAX77803_MUIC_DOCK_HMT);
		break;
	case ADC_SMARTDOCK:
		/* Smart Dock */
...
static void max77803_muic_detach_hmt(struct max77803_muic_info *info)
{
	struct max77803_muic_data *mdata = info->muic_data;
	enum cable_type_muic tmp_cable_type = info->cable_type;
...
static void max77803_muic_attach_hmt(struct max77803_muic_info *info,
						u8 adc, u8 vbvolt, u8 chgtyp)
{
	struct max77803_muic_data *mdata = info->muic_data;
				       ...
				       u8 status1, u8 status2, int irq)
{
	...
	case ADC_SMARTDOCK:
		...
		break;
	case ADC_HMT:
		max77803_muic_attach_hmt(info, adc, vbvolt, chgtyp);
		break;
	case ADC_VZW_USB_DOCK:
	case ADC_AUDIODOCK:
		if(!!vbvolt)
