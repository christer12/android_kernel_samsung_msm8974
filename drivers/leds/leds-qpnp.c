...
void tkey_led_enables(int level)
{
	...
	if (level == 1)
		brightness = RGB_MAX_LEVEL;
	else if (level == 0)
		brightness = LED_OFF;
	else
		brightness = level;

	qpnp_led_set(&led_array[BLUE].cdev, brightness);
	qpnp_led_set(&led_array[GREEN].cdev, brightness);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	for ( i = RED; i <= BLUE; i++) {
		qpnp_led_set(&led_array[i].cdev,(long)brightness);
	}
	ret = kstrtol(buf, 10, &brightness);
	if(ret){
		printk(KERN_ERR "[LED] can't convert str to l\n");
		return count;
	}
	if (ret < 0 ) {
		printk("[LED] Cant set brightness\n");
		return count;
	}
	...
	printk("led_r is set with brightness value %ld\n",brightness);

	qpnp_led_set(&led_array[RED].cdev,(long)brightness);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("led_g is set with brightness value %ld\n",brightness);
	qpnp_led_set(&led_array[GREEN].cdev, (long)brightness);
	return count;
}
                ...
                struct device_attribute *attr, char *buf)
{
	struct qpnp_led_data *led_array;
	led_array = dev_get_drvdata(dev);
        return snprintf(buf, 4, "%d\n", led_array[GREEN].cdev.brightness );
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("led_b is set with brightness value %ld\n",brightness);
	qpnp_led_set(&led_array[BLUE].cdev, (long)brightness);
                ...
                struct device_attribute *attr, char *buf)
{
	struct qpnp_led_data *led_array;
	led_array = dev_get_drvdata(dev);
        return snprintf(buf, 4, "%d\n", led_array[BLUE].cdev.brightness );
}
...
static int __devinit qpnp_leds_probe(struct spmi_device *spmi)
{
	...
	if ( led_array[RED].id == QPNP_ID_RGB_RED && led_array[GREEN].id == QPNP_ID_RGB_GREEN && led_array[BLUE].id == QPNP_ID_RGB_BLUE  ) {
