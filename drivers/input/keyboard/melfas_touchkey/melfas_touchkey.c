...
#define MCS5080_last_ver	0x60 //0x59 // 0x09  /*M550 v07*/
...
static u8 two_touch_speed = 50; //30;
...
#include <linux/wakelock.h>  // jgb
struct	wake_lock		report_wake_lock;  // jgb
...
void melfas_i2c_power_onoff(int onoff);
			...
			else // key pressed
			{
				...
				else
				{
					...
					if(keycode == TOUCHKEY_KEYCODE_WAKE){
						...
						wake_lock_timeout(&report_wake_lock, 3*HZ); // jgb
...
static int i2c_touchkey_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk(KERN_ERR "[TKEY] %s: power on, %d\n",__func__, __LINE__);
	if (!vddo_vreg) {
		vddo_vreg = regulator_get(&pdata->client->dev, "vcc_en");
		if (IS_ERR(vddo_vreg)) {
			rc = PTR_ERR(vddo_vreg);
			printk(KERN_ERR "[TKEY] Regulator(vcc_en) get failed rc=%d\n", rc);
		}
		if (regulator_count_voltages(vddo_vreg) > 0) {
			rc = regulator_set_voltage(vddo_vreg,1800000, 1800000);
			if (rc) {
				printk(KERN_ERR "[TKEY] regulator(vcc_en) set_vtg failed rc=%d\n",rc);
			}
		}
	}
	init_hw();
	...
	wake_lock_init(&report_wake_lock, WAKE_LOCK_SUSPEND,"report_wake_lock"); // jgb
...
void melfas_i2c_power_onoff(int onoff)
{
	...
	printk(KERN_ERR "[TKEY] power 2 : on:%d (%d)\n", onoff,__LINE__);

	if (onoff) {
			rc = reg_set_optimum_mode_check(vddo_vreg,
						10000);
			if (rc < 0) {
				printk(KERN_ERR "Regulator vcc_en set_opt failed rc=%d\n",
				rc);
			...
			if (rc) {
				printk(KERN_ERR "Regulator vcc_en enable failed rc=%d\n",
				rc);
				goto error_reg_en_vcc_en;
			}
	} else {
			reg_set_optimum_mode_check(vddo_vreg, 0);
			regulator_disable(vddo_vreg);
	}
	msleep(50);
...
error_reg_opt_i2c:
//error_set_vtg_i2c:
//	regulator_put(vddo_vreg); // pdata->vcc_en);
//error_get_vtg_i2c:
	return;
}
...
{
	int ret;
	printk(KERN_ERR "[TKEY] power 1 : on:%d (%d)\n", onoff,__LINE__);
