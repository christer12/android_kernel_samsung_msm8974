...
#define TC360_FW_NAME "vikal_coreriver_tkey"
...
#define TC360_FW_BUILTIN_PATH	"tkey"
#define TC360_FW_IN_SDCARD_PATH	"/sdcard/"
...
#define TC360_KEY_INDEX_MASK	0x07
#define TC360_KEY_PRESS_MASK	0x08

#define TC360_CMD		0x00
#define TC360_CMD_FW_VER	0x01
#define TC360_CMD_MODULE_VER	0x02
#define TC360_CMD_RAW_MENU	0x14
#define TC360_CMD_RAW_BACK	0x04
#ifdef HOME_KEY_USE
#define TC360_CMD_RAW_HOME	0x0C
#endif
...
static struct tc360_data *tkey_driver=NULL;
...
static int tc360_keycodes[]={KEY_BACK,KEY_HOMEPAGE,KEY_MENU};

void set_tkey_info(struct tc360_data *data)
{
	if(data != NULL)
		tkey_driver = data;
	else
		pr_info("%s : data is NULL",__func__);
}

static struct tc360_data * get_tkey_info(void)
{
	return tkey_driver;
}
...
void tc360_power(struct tc360_data *info, int onoff)
{
	...
	touchkey_pmic_control(onoff);
	if (onoff) {
		if (!regulator_is_enabled(info->vcc_en)) {
			ret = regulator_enable(info->vcc_en);
			if (ret) {
				dev_err(&info->client->dev,
					"Regulator vcc_en enable failed rc=%d\n", ret);
				return;
			}
			dev_info(&info->client->dev,"tkey 2.2V[vcc_en] on is finished.\n");
		} else
			dev_info(&info->client->dev,"tkey 2.2V[vcc_en] is already on.\n");
	} else {
		if (regulator_is_enabled(info->vcc_en)) {
			ret = regulator_disable(info->vcc_en);
			if (ret) {
				dev_err(&info->client->dev,
					"Regulator vcc_en disable failed rc=%d\n", ret);
				return;
			}
			dev_info(&info->client->dev,"tkey 2.2V[vcc_en] off is finished.\n");
		} else
			dev_info(&info->client->dev,"tkey 2.2V[vcc_en] is already off.\n");
	}
}
...
{
	int ret;
	printk(KERN_ERR "[TKEY] power : on:%d (%d)\n", onoff,__LINE__);
	...
	if(!sub_ldo2){
		...
		if(sub_ldo2 != NULL)
		{
			ret = regulator_set_voltage(sub_ldo2, 3000000, 3000000);
			if (ret) 
	...
	if(sub_ldo2 != NULL){
		...
		pr_err("sub_ldo2 is null, could not control \n");
	}
			...
			struct tc360_platform_data *pdata)
{
	...
	pdata->udelay=6;
	pdata->suspend_type=TC360_SUSPEND_WITH_POWER_OFF;	
...
static int tc360_flash_fw(struct tc360_data *data, u8 fw_path, bool force)
{
	...
	dev_info(&client->dev, "Module ver (%#x)\n", module_ver);
...
#ifdef HOME_KEY_USE
static ssize_t fac_read_chpct_home_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	struct tc360_data *data = dev_get_drvdata(dev);
	...
	ret = i2c_smbus_read_i2c_block_data(client, TC360_CMD_RAW_HOME,
					    TC360_RAW_DATA_SIZE, buff);
	if (ret < 0) {
		dev_err(&client->dev, "%s: failed to read raw data (%d)\n",
			__func__, ret);
...
#ifdef HOME_KEY_USE
static ssize_t fac_read_raw_home_show(struct device *dev,
				      struct device_attribute *attr,
				      char *buf)
{
	struct tc360_data *data = dev_get_drvdata(dev);
...
#ifdef HOME_KEY_USE
static DEVICE_ATTR(touchkey_home, S_IRUGO,
		   fac_read_chpct_home_show, NULL);
#endif
...
#ifdef HOME_KEY_USE
static DEVICE_ATTR(touchkey_raw_data2, S_IRUGO, fac_read_raw_home_show, NULL);
#endif
...
{ 	
	...
	/* input_touchkey_init ...*/
	gpio_tlmm_config(GPIO_CFG(95,0, GPIO_CFG_INPUT,GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
	gpio_tlmm_config(GPIO_CFG(96,0, GPIO_CFG_INPUT,GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
	gpio_tlmm_config(GPIO_CFG(29,0, GPIO_CFG_INPUT,GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
	...
	if (client->irq) {
		ret = request_threaded_irq(client->irq, NULL, tc360_interrupt,
					IRQF_TRIGGER_FALLING | IRQF_ONESHOT,TC360_DEVICE, data);
		if (ret) {
...
static int tc360_resume(struct device *dev)
{
	...
	if(unlikely(data->flip_status==FLIP_OPEN)) {
		dev_info(&client->dev, "%s, flip opened.\n", __func__);
		goto out;
	}
	...
	if(data->led_wq_passed!=1 && data->led_brightness!=LED_OFF){
		dev_info(&client->dev, "%s : led wq isn't called because of late power on.\n",__func__);
		ret = i2c_smbus_write_byte_data(client, TC360_CMD, TC360_CMD_LED_ON);
		if (ret < 0)
			dev_err(&client->dev, "%s: failed to wt led data (%d)\n",__func__, ret);
	}
...
void samsung_switching_tkey(int flip)
{
	...
	printk(KERN_ERR "tc360 [TKEY] %s : flip_status : %d, flip : %d, enabled : %d\n", __func__,data->flip_status, flip,data->enabled);
	
	if (data->flip_status!= flip)
	{
		data->flip_status=flip;
		set_tkey_info(data);
		if (flip == FLIP_CLOSE) {
			pr_info("tc360 [TKEY] %s : flip closed. tkey must be resumed.\n",__func__);
			tc360_resume(&data->client->dev);
		}
		else {
			pr_info("tc360 [TKEY] %s : flip opened. tkey must be suspended\n",__func__);
