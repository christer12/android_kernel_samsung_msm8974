		...
		const char *buf, size_t count)
{
	...
	if(rmi4_data->ftouch_reboot == 1){
		dev_err(&rmi4_data->i2c_client->dev, "%s: Skip cmd during rebooting :%s\n",
			 __func__, buf);
		return count;
	}
#endif
		...
		const struct firmware *fw_entry)
{
	...
	dev_info(&rmi4_data->i2c_client->dev, "%s: [FW size. revision, version] [%d, 0x%02X/0x%02X(BIN/IC), 0x%02X/0x%02X(BIN/IC)]\n",
	...
	/* Discard 58&59 f/w, so this routine may be removed several weeks later */
	if((rmi4_data->fw_version_of_ic == 0x58)||(rmi4_data->fw_version_of_ic == 0x59)){
		dev_err(&rmi4_data->i2c_client->dev, "%s: Force firmware down to 0x56\n",
			__func__);
		return false;
	}
	...
	if (!rmi4_data->fw_version_of_ic
		...
		dev_info(&rmi4_data->i2c_client->dev, "%s:[TEST] Firmware is TEST firmware\n",
			__func__);
		return true;
	}
#endif
	...
	if ((rmi4_data->ic_revision_of_ic == 0xB0) && (!rmi4_data->bootmode)) {
	...
	if (0 < fw_size) {
		...
		if (nread != fw_size) {
			...
			int fw_version_of_bin =
				(int)fw_data[FW_VERSION_BIN_OFFSET];
