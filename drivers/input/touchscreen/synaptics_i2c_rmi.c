...
#define PATTERN_TRACKING_DISTANCE 8
#else
...
static bool tsp_pattern_tracking(int fingerindex, int x, int y, bool movecheck);
#endif
...
static bool tsp_pattern_tracking(int fingerindex, int x, int y, bool movecheck)
{
	int max_ghosttouch_th = MAX_GHOSTTOUCH_COUNT;
	bool ghosttouch  = false;

	#ifdef PATTERN_TRACKING_FOR_FULLSCREEN
	if(!IsEdgeArea(x,y)){
		if(movecheck == true) return ghosttouch;
		max_ghosttouch_th = MAX_GHOSTTOUCH_COUNT*2;
	}
	#else
	...
	if(diff_two_point(x,y, touchbx[fingerindex], touchby[fingerindex]))
			{
		tcount_finger[fingerindex]++;
			}
			else
			{
		tcount_finger[fingerindex] = 0;
			}

	touchbx[fingerindex] = x;
	touchby[fingerindex] = y;

	if(tcount_finger[fingerindex]>= max_ghosttouch_th){
				clear_tcount();
				ghosttouch = true;
				return ghosttouch;
			}
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			if (!rmi4_data->finger[finger].state) {
				...
				if(tsp_pattern_tracking(finger,x,y,false)){
					dev_err(&rmi4_data->i2c_client->dev,
				...
				if((rmi4_data->finger[finger].mcount%MOVE_COUNT_TH) == 0){
					if(tsp_pattern_tracking(finger,x,y,true)){
						dev_err(&rmi4_data->i2c_client->dev,
...
static void synaptics_rmi4_reboot_work(struct work_struct *work)
{
	...
	if(rmi4_data->ftouch_reboot == 1) return;
	rmi4_data->ftouch_reboot  = 1;
		...
		const struct i2c_device_id *dev_id)
{
	...
	if(touch_fpcb_version == OCTA_PANEL_REVISION_51){
		...
		rmi4_data->board->fac_firmware_name = FAC_FWIMAGE_NAME_B0_5_1;
	}
	else{
		rmi4_data->panel_revision = OCTA_PANEL_REVISION_43;
...
static int synaptics_rmi4_stop_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	mdss_dsi_panel_touchsensing(false);
