...
static struct mxt_platform_data mxt_platformdata = {
	.num_xnode = 27,
	.num_ynode = 15,
	.max_x = 4096,
	.max_y = 4096,
...
#if ENABLE_TOUCH_KEY
	.num_touchkey = ARRAY_SIZE(mxt_touchkey_data),
	.touchkey = mxt_touchkey_data,
#endif

};

static int mxt_request_gpio(struct mxt_power_data *pdata)
{
	int ret;
	printk(KERN_INFO "%s\n", __func__);

	ret = gpio_request(pdata->tsp_rst, "mxts,tsp_rst");
	if (ret) {
		pr_err("%s: unable to request tsp_rst [%d]\n",
				__func__, pdata->tsp_en);
		return ret;
	}
	gpio_tlmm_config(GPIO_CFG(pdata->tsp_rst, 0,
		GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);

	ret = gpio_request(pdata->tsp_en, "mxts,tsppwr_en");
	if (ret) {
		pr_err("%s: unable to request tsppwer_en [%d]\n",
				__func__, pdata->tsp_en);
		return ret;
	}
	gpio_tlmm_config(GPIO_CFG(pdata->tsp_en, 0,
		GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);

	ret = gpio_request(pdata->tsp_en1, "mxts,tsppwr_en1");
	if (ret) {
		pr_err("%s: unable to request tsppwer_en1 [%d]\n",
				__func__, pdata->tsp_en1);
		return ret;
	}
	gpio_tlmm_config(GPIO_CFG(pdata->tsp_en1, 0,
		GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), 1);
...
static int mxt_power_onoff(struct mxt_data *data, bool enable)
{
	...
	dev_err(&data->client->dev,
			"%s enable(%d)\n", __func__, enable);
	ret = gpio_direction_output(data->pwrdata->tsp_en, enable);
	if (ret) {
		dev_err(&data->client->dev,
			"[TKEY]%s: unable to set_direction for vdd_led [%d]\n",
			 __func__, data->pwrdata->tsp_en);
		return -1;
	}
	ret = gpio_direction_output(data->pwrdata->tsp_en1, enable);
	if (ret) {
		dev_err(&data->client->dev,
			"[TKEY]%s: unable to set_direction for vdd_led [%d]\n",
			 __func__, data->pwrdata->tsp_en1);
		return -1;
	}

	msleep(10);

	ret = gpio_direction_output(data->pwrdata->tsp_rst, enable);
	if (ret) {
		dev_err(&data->client->dev,
...
static irqreturn_t mxt_irq_thread(int irq, void *ptr)
{
	...
	} while (!gpio_get_value(data->pwrdata->tsp_int));
 ...
 */
	...
	if (system_rev == 2) {	/* Check TSP Ver 0.1 and Ver 0.2 for Rev 0.2. */
		...
		dev_info(&data->client->dev,
					"HW_REV 0.2, X27_ref=[%d],X26_ref=[%d]",
					ref_x27, ref_x26);
		...
		if (ref_x26 < 600 && ref_x27 < 600) {
			...
			dev_info(&data->client->dev,
					"HW_REV 0.2 and TSP_REV 0.2 change keycode!");
		} else {
			dev_info(&data->client->dev,
					"HW_REV 0.2 and TSP_REV 0.1 keep current state!");
		}
		...
		dev_info(&data->client->dev,
				"HW_REV 0.3, X27_ref=[%d],X26_ref=[%d]",
				ref_x27, ref_x26);

		if (ref_x26 < 600 && ref_x27 < 600) {
			mxt_write_object(data, MXT_TOUCH_KEYARRAY_T15,
						MXT_KEYARRY_XORIGIN, 0);
			dev_info(&data->client->dev,
					"HW_REV 0.3 and TSP_REV 0.3 change X-origin!");
		} else {
			dev_info(&data->client->dev,
					"HW_REV 0.3 and TSP_REV 0.4 keep current state!");
		}
	}
#else
	dev_info(&data->client->dev,
			"%s: Not support TSP_SEC_FACTORY!", __func__);
...
static int mxt_flash_fw_on_probe(struct mxt_fw_info *fw_info)
{
	...
	if (data->info.version == fw_info->fw_ver
		...
		dev_info(dev, "Firmware version is same with in IC[IC:0x%x,0x%x / FW:0x%x,0x%x]\n",
				data->info.version, data->info.build, fw_info->fw_ver, fw_info->build_ver);
		goto out;
	}
...
static int __devinit mxt_touch_init(struct mxt_data *data, bool nowait)
{
	...
	const char *firmware_name =
		 data->pdata->firmware_name ?: MXT_V_PROJECT_FIRMWARE_NAME;
	int ret = 0;
		...
		const struct i2c_device_id *id)
{
	...
	/*const struct mxt_platform_data = client->dev.platform_data;*/
	struct mxt_power_data *power_data = NULL;
	struct mxt_data *data;
	...
	pdata = &mxt_platformdata;
	printk(KERN_ERR "%s", __func__);

	if (client->dev.of_node) {
		power_data = devm_kzalloc(&client->dev,
			sizeof(struct mxt_power_data),
				GFP_KERNEL);
		if (!pdata) {
			dev_err(&client->dev,
		...
		error = mxt_parse_dt(&client->dev, power_data);
