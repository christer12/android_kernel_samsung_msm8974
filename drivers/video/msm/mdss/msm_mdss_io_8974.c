			    ...
			    char frame_rate)
{
	...
	if ((panel_info->mipi.dst_format == DSI_CMD_DST_FORMAT_RGB888)
	    || (panel_info->mipi.dst_format == DSI_VIDEO_DST_FORMAT_RGB888)
	    || (panel_info->mipi.dst_format
		== DSI_VIDEO_DST_FORMAT_RGB666_LOOSE))
		bpp = 3;
	else if ((panel_info->mipi.dst_format == DSI_CMD_DST_FORMAT_RGB565)
		 || (panel_info->mipi.dst_format
	...
	h_period = ((panel_info->lcdc.h_pulse_width)
			+ (panel_info->lcdc.h_back_porch)
			+ (panel_info->xres)
			+ (panel_info->lcdc.h_front_porch));

	v_period = ((panel_info->lcdc.v_pulse_width)
	...
	if ((frame_rate !=
		...
		if (lanes > 0) {
			panel_info->clk_rate =
			((h_period * v_period *
			  (panel_info->mipi.frame_rate) * bpp * 8)
			   / lanes);
		} else {
...
void mdss_dsi_clk_enable(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (!ctrl_pdata->panel_data.panel_info.cont_splash_enabled) {
		//WARN(1, "could not attach iommu client %s to ctx \n",__func__);
	...
	/* Reset any PLL unlock status bits */
	MIPI_OUTP(ctrl_pdata->ctrl_base + 0x012c,
			MIPI_INP(ctrl_pdata->ctrl_base + 0x12c) | BIT(17));
	pr_debug("%s: Dumping reg(0x12c) = 0x%x",__func__, MIPI_INP(ctrl_pdata->ctrl_base + 0x12c));
...
void mdss_edp_clk_enable(struct mdss_edp_drv_pdata *edp_drv)
{
	...
	if (clk_set_rate(edp_drv->pixel_clk, 268500000) < 0)
		...
		unsigned char *mmss_cc_base, int enable)
{
	...
	edp_write(mmss_cc_base + 0x00a8, 0x6b); /* M value */
	edp_write(mmss_cc_base + 0x00ac, 0x5d); /* N value */
...
void mdss_edp_phy_misc_cfg(unsigned char *edp_base)
{
	...
	edp_write(edp_base + 0x514, 0x6E);
	/* EDP_PHY_EDPPHY_GLB_MISC9 */
	edp_write(edp_base + 0x518, 0x6c);
	/* EDP_MISC1_MISC0 */
	edp_write(edp_base + 0x2c, 0x21);
}
