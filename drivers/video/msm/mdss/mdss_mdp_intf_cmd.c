...
static int mdss_mdp_cmd_tearcheck_setup(struct mdss_mdp_ctl *ctl, int enable)
{
	...
	if (pinfo->mipi.vsync_enable && enable) {
		...
		/* 3 ms kickoff window start from TE */
		ctx->start_threshold = pinfo->yres * 3 / 17;
...
static int mdss_mdp_cmd_wait4comp(struct mdss_mdp_ctl *ctl, void *arg)
{
	...
	if (board_rev > 0x02) {
		rc = wait_for_completion_interruptible_timeout(
			&ctx->vsync_comp, KOFF_TIMEOUT);
	} else {
		rc = wait_for_completion_interruptible_timeout(
			&ctx->vsync_comp, msecs_to_jiffies(20));
	}
...
int mdss_mdp_cmd_stop(struct mdss_mdp_ctl *ctl)
{
	...
	mutex_lock(&ctx->clk_mtx);
	if (ctx->vsync_enabled || ctx->clk_enabled) {
...
static int mdss_mdp_cmd_vsync_remove(struct mdss_mdp_ctl *ctl,
			struct mdss_mdp_vsync_handler *handler)
{
	struct mdss_mdp_cmd_ctx *ctx;
	struct mdss_mdp_vsync_handler null_handle;
...
int mdss_mdp_cmd_start(struct mdss_mdp_ctl *ctl)
{
	...
	ctl->remove_vsync_handler = mdss_mdp_cmd_vsync_remove;
