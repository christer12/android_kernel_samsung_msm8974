...
static int set_hdmi_mode(struct sii8240_data *sii8240, bool hdmi_mode)
{
	...
	if (hdmi_mode) {
		...
		pr_err("sii8240: %s():%d [jgk] BIT_RX_HDMI_CTRL2_USE_AV_MUTE_SUPPORT_ENABLE ###\n",	__func__, __LINE__);
#else/* workaround for AV_SET_MUTE from Qualcomm */
		ret = mhl_modify_reg(hdmi, 0xA3,
			BIT_RX_HDMI_CTRL2_USE_AV_MUTE_SUPPORT_MASK,
			BIT_RX_HDMI_CTRL2_USE_AV_MUTE_SUPPORT_DISABLE);
		pr_err("sii8240: %s():%d [jgk] BIT_RX_HDMI_CTRL2_USE_AV_MUTE_SUPPORT_DISABLE ###\n",	__func__, __LINE__);
#endif
		if (unlikely(ret < 0)) {
			pr_err("[ERROR] sii8240: %s():%d failed !\n",
					__func__, __LINE__);
...
static void sii8240_power_down(struct sii8240_data *sii8240)
{
	...
	ret = mhl_write_byte_reg(disc, POWER_CTRL_REG, 0);
	if (unlikely(ret < 0)) {
		pr_err("[ERROR]sii8240: %s():%d failed !\n",
				__func__, __LINE__);
	}
...
static int sii8240_packed_pixel_avi_info_locked(struct sii8240_data *sii8240)
{
	...
	if (vic <= HDMI_VFRMT_720x480p60_16_9 && vic != 0) {
		pr_err("[ERROR] sii8240: under 75MHz\n");
		ret = mhl_write_byte_reg(hdmi, 0x4C, 0xD0);
		if (unlikely(ret < 0)) {
			pr_err("[ERROR] sii8240: %s():%d mhl_write_byte_reg\n",
					__func__, __LINE__);
...
unhandled:
	...
	pr_err("[ERROR] disc intr1 0x%x, rgnd 0x%x\n", intr1, rgnd);
...
static void charger_mhl_cb(bool otg_enable, int charger)
{
	struct sii8240_data *sii8240 = dev_get_drvdata(sii8240_mhldev);
...
static void of_sii8240_gpio_config(void)
{
	struct sii8240_data *sii8240 = dev_get_drvdata(sii8240_mhldev);
...
/*	blocking_notifier_chain_register(&acc_mhl_notifier, &sii8240->mhl_nb);*/
			...
			const struct i2c_device_id *id)
{
	...
	if (0 == client_id) {
		...
		pdata->vbus_present = charger_mhl_cb;
#ifdef CONFIG_MFD_MAX77803
