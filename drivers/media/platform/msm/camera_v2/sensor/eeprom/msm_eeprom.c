...
static int msm_eeprom_match_caldata_checksum(struct msm_eeprom_ctrl_t *e_ctrl)
{
	uint8_t *memptr;
	int j, rc;
	struct msm_eeprom_board_info *eb_info = NULL;
	...
	for (j = 0; j + 1 < eb_info->num_blocks; j += 2) {
		/* empty table or no checksum */
		if (!emap[j].mem.valid_size || !emap[j+1].mem.valid_size) {
			memptr += emap[j].mem.valid_size
				+ emap[j+1].mem.valid_size;
			continue;
		}
		if (emap[j+1].mem.valid_size != sizeof(uint32_t)) {
			CDBG("%s: malformatted data mapping\n", __func__);
			return -EINVAL;
		}
		sum = (uint32_t *) (memptr + emap[j].mem.valid_size);
		rc = msm_eeprom_verify_sum(memptr, emap[j].mem.valid_size,
					   *sum);
		if (!rc)
			e_ctrl->is_supported |= 1 << (j / 2 + 1);
		memptr += emap[j].mem.valid_size + emap[j+1].mem.valid_size;
	}
...
static int msm_eeprom_spi_setup(struct spi_device *spi)
{
	...
	eb_info = kzalloc(sizeof(struct msm_eeprom_board_info), GFP_KERNEL);
	if (!eb_info)
		goto spi_free;
	...
	msm_eeprom_match_caldata_checksum(e_ctrl);
