...
ERROR2:
	...
	sensordata->csi_lane_params = NULL;
ERROR1:
	return rc;
}
	...
	struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (!sensordata->sensor_init_params) {
		pr_err("%s failed %d\n", __func__, __LINE__);
		kfree(s_ctrl->sensordata);   // memory free
...
ERROR9:
	...
	s_ctrl->sensordata->slave_info = NULL;
ERROR8:
	kfree(s_ctrl->sensordata->actuator_info);
	s_ctrl->sensordata->actuator_info = NULL;
ERROR7:
	kfree(s_ctrl->sensordata->gpio_conf->gpio_num_info);
	s_ctrl->sensordata->gpio_conf->gpio_num_info = NULL;
ERROR6:
	kfree(s_ctrl->sensordata->gpio_conf->cam_gpio_set_tbl);
	s_ctrl->sensordata->gpio_conf->cam_gpio_set_tbl = NULL;
ERROR5:
	kfree(s_ctrl->sensordata->gpio_conf->cam_gpio_req_tbl);
	s_ctrl->sensordata->gpio_conf->cam_gpio_req_tbl = NULL;
ERROR4:
	kfree(s_ctrl->sensordata->gpio_conf);
	s_ctrl->sensordata->gpio_conf = NULL;
ERROR3:
	kfree(s_ctrl->sensordata->cam_vreg);
	s_ctrl->sensordata->cam_vreg = NULL;
ERROR2:
	kfree(s_ctrl->sensordata->csi_lane_params);
	s_ctrl->sensordata->csi_lane_params = NULL;
ERROR1:
	kfree(sensordata->sensor_init_params); //memory free
	sensordata->sensor_init_params = NULL;
...
int32_t msm_sensor_free_sensor_data(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	s_ctrl->sensordata->sensor_info = NULL;
	kfree(s_ctrl->sensordata->sensor_init_params);
	s_ctrl->sensordata->sensor_init_params = NULL;
	kfree(s_ctrl->sensordata);
	s_ctrl->sensordata = NULL;
	return 0;
}
...
int32_t msm_sensor_power_up(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	struct msm_camera_sensor_board_info *data = NULL;
	struct msm_sensor_power_setting *power_off_setting = NULL;
	int32_t off_index = 0;

	if (!s_ctrl || !s_ctrl->pdev || !s_ctrl->dev) {
	...
	for (index = 0; index < power_setting_array->size; index++) {
		...
		case SENSOR_CLK:
			...
			if (power_setting_array->power_off_setting) {
			    for (off_index = 0; off_index < power_setting_array->off_size; off_index++) {
				power_off_setting = &power_setting_array->power_off_setting[off_index];
				if (power_off_setting->seq_type == SENSOR_CLK) {
				    CDBG("check clk %s,%d, %d!!!!",
					 s_ctrl->clk_info[0].clk_name,
					 index, off_index);
		...
		case SENSOR_VREG:
			...
			case CAM_VDIG:
			    CDBG("power val is CAM_VDIG");
			    break;
			case CAM_VIO:
			    CDBG("power val is CAM_VIO");
			    break;
			case CAM_VAF:
			    CDBG("power val is CAM_VAF");
			    break;
			case CAM_VANA:
			    CDBG("power val is CAM_VANA");
			    break;
			default:
			    CDBG("power val is nothing!");
			    break;
			}
		...
		if (power_setting->delay > 20) {
			CDBG("sleep for power sequency : %d",power_setting->delay);
...
int32_t msm_sensor_power_down(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->check_power_on)
	    s_ctrl->check_power_on = false;
	else {
	...
	if (power_setting_array->power_off_setting) {
	    CDBG("%s uses power off setting", s_ctrl->sensordata->sensor_name);
	    t_save_for_power_off = power_setting_array->power_setting;
	    l_save_for_power_off = power_setting_array->size;
	    power_setting_array->power_setting = power_setting_array->power_off_setting;
	    power_setting_array->size = power_setting_array->off_size;
	}
	...
	if (power_setting_array->power_off_setting) {
	    CDBG("%s recovery power setting", s_ctrl->sensordata->sensor_name);
	    power_setting_array->power_setting = t_save_for_power_off;
	    power_setting_array->size = l_save_for_power_off;
	}
...
static int32_t msm_sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	if (s_ctrl->free_power_setting == true) {
		...
		s_ctrl->power_setting_array.power_setting = NULL;
