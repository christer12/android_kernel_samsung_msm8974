...
static int aux_pcm_rx;
static int aux_pcm_tx;
				...
				struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		if (aux_pcm_rx == 0) {
			dev_err(dai->dev, "%s: No AUX_PCM_Rx Path active!!"
			" return, dai->id %d aux_pcm_count = %d\n",
		...
		if (aux_pcm_tx == 0) {
			dev_err(dai->dev, "%s: No AUX_PCM_Tx Path active!!"
			" return, dai->id %d aux_pcm_count = %d\n",
			__func__, dai->id, aux_pcm_count);
			mutex_unlock(&aux_pcm_mutex);
			return;
		...
		struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		dev_err(dai->dev, "%s: Activating AUX_PCM_Rx Path,"
			"dai->id %d aux_pcm_count = %d\n",
			__func__, dai->id, aux_pcm_count);
		aux_pcm_rx++;
	}
	else {
		dev_err(dai->dev, "%s: Activating AUX_PCM_Tx Path,"
			"dai->id %d aux_pcm_count = %d\n",
			__func__, dai->id, aux_pcm_count);
		aux_pcm_tx++;
	}
