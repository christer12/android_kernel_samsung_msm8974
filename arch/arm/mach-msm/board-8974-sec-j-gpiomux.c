...
#if !defined(CONFIG_MACH_KS01EUR) /* NOT USE DMB */
	{
...
static struct msm_gpiomux_config nc_rev_configs[] __initdata = {
	{
		.gpio = 18,
		.settings = {
...
#if defined(CONFIG_MACH_KS01EUR)
	if (system_rev < 16) {
	msm_gpiomux_install(nc_rev_configs,
			ARRAY_SIZE(nc_rev_configs));
	}
#endif
}
