...
#ifdef DEBUG_RMT_STORAGE
#define MAX_NUM_RMT_STORAGE_WAKELOCK_NAME (5)
#define MAX_RMT_STORAGE_WAKELOCK_FUNC_NAME (50)
typedef struct {
	char wakelock_name[MAX_WAKELOCK_NAME_SZ];
	struct wake_lock * p_wake_lock;
	char locked_func[MAX_RMT_STORAGE_WAKELOCK_FUNC_NAME];
} dbg_wakelock_rmt_t;
dbg_wakelock_rmt_t dbg_wakelock_rmt_storage[MAX_NUM_RMT_STORAGE_WAKELOCK_NAME];
uint32_t dbg_wakelock_rmt_storage_idx;
#endif
...
#ifdef DEBUG_RMT_STORAGE
		for(idx=0; idx<dbg_wakelock_rmt_storage_idx; idx++) {
			if(dbg_wakelock_rmt_storage[idx].p_wake_lock == &port_ptr->port_rx_wake_lock) {
				snprintf(dbg_wakelock_rmt_storage[idx].locked_func,
						MAX_RMT_STORAGE_WAKELOCK_FUNC_NAME, "%s", __func__);
				pr_info("[DEBUG]rmt_storage: %s locked, %s\n", dbg_wakelock_rmt_storage[idx].wakelock_name, __func__);
				break;
			}
		}
#endif
...
#ifdef DEBUG_RMT_STORAGE
	if(!strncmp(current->comm, "rmt_storage", strlen("rmt_storage"))) {
		if(dbg_wakelock_rmt_storage_idx < MAX_NUM_RMT_STORAGE_WAKELOCK_NAME) {
			snprintf(dbg_wakelock_rmt_storage[dbg_wakelock_rmt_storage_idx].wakelock_name,
				MAX_WAKELOCK_NAME_SZ, "%s", port_ptr->rx_wakelock_name);

			dbg_wakelock_rmt_storage[dbg_wakelock_rmt_storage_idx++].p_wake_lock
				= &port_ptr->port_rx_wake_lock;
			
			pr_info("[DEBUG]rmt_storage: %s is initalized\n", port_ptr->rx_wakelock_name);
		} else
			pr_info("[DEBUG]rmt_storage: can't store %s\n", port_ptr->rx_wakelock_name);
	}
#endif
			...
			size_t buf_len)
{
	...
	if (list_empty(&port_ptr->port_rx_q)) {
		...
		for(idx=0; idx<dbg_wakelock_rmt_storage_idx; idx++) {
			if(dbg_wakelock_rmt_storage[idx].p_wake_lock == &port_ptr->port_rx_wake_lock) {
				snprintf(dbg_wakelock_rmt_storage[idx].locked_func, MAX_RMT_STORAGE_WAKELOCK_FUNC_NAME,
						"unlock");
				pr_info("[DEBUG]rmt_storage: %s unlock, %s\n", dbg_wakelock_rmt_storage[idx].wakelock_name, __func__);
				break;
			}
		}
#endif
...
int msm_ipc_router_close_port(struct msm_ipc_port *port_ptr)
{
	...
	for(idx=0; idx<dbg_wakelock_rmt_storage_idx; idx++) {
		if(dbg_wakelock_rmt_storage[idx].p_wake_lock == &port_ptr->port_rx_wake_lock) {
			snprintf(dbg_wakelock_rmt_storage[idx].locked_func, MAX_RMT_STORAGE_WAKELOCK_FUNC_NAME,
					"destroy");
			pr_info("[DEBUG]rmt_storage: %s destroy, %s\n", dbg_wakelock_rmt_storage[idx].wakelock_name, __func__);
			break;
		}
	}
#endif
